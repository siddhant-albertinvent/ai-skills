...
#
# Clones the ai-skills repo into .githooks/ inside the target project and
# configures Git to use it as the hooks directory.
#
...
for hook in "$HOOKS_PATH"/*; do
  if [ -f "$hook" ] && [[ "$(basename "$hook")" =~ ^(commit-msg|pre-commit|pre-push|post-merge|prepare-commit-msg)$ ]]; then
    chmod +x "$hook"
...
echo "    curl -fsSL https://raw.githubusercontent.com/siddhant-albertinvent/ai-skills/main/setup/github/githooks.sh | bash"
echo ""
